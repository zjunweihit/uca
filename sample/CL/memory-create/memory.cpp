...
int main(int argc, char *argv[])
{
	...
	mem_obj[0] = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
	...
	// mapping input memory obj mem_obj[0]
	int *mapPtr = (int*) clEnqueueMapBuffer(cmdq, mem_obj[0], CL_TRUE, CL_MAP_WRITE,
				0, sizeof(int) * ARRAY_SIZE, 0, NULL, NULL, &err);
	if (checkErr(err, "failed to map buffer 0!") != CL_SUCCESS) {
		Cleanup(ctx, cmdq, prog, kernel, mem_obj);
		return err;
	}
	for (int i = 0; i < ARRAY_SIZE; i++)
